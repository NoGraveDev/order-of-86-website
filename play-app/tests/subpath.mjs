...
try{
 ...
 const entry=await (await fetch(base+'/play/multiplayer')).text();assert(entry.includes('id="publicJoin"'));assert(entry.includes('src="entry.js"'));assert(!entry.includes('src="loading-screen.js"'));assert(entry.includes('entry-locked multiplayer-session'));const soloEntry=await(await fetch(base+'/play/')).text();assert(soloEntry.includes('src="entry.js"'));assert(soloEntry.includes('/site-metrics.js'));assert.equal((await fetch(base+'/play/entry.css')).status,200);

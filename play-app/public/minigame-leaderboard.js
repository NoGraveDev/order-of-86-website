...
export function ensureMinigamePanels(){
 ...
 panel.insertBefore(card('inventoryCrosswordRecord','Violet Crossword'),$('inventoryMinigameStatus'));
...
export function renderMinigamePanels(records){ensureMinigamePanels();if(!$('inventoryBoatRecord'))return;const boat=$('inventoryBoatRecord'),lava=$('inventoryLavaRecord');boat.replaceChildren();lava.replaceChildren();
 const crossword=$('inventoryCrosswordRecord');crossword.replaceChildren();stats(crossword,[[records.crossword.count,'Completed puzzles'],[time(records.crossword.bestTime),'Personal best']]);const crosswordHistory=element('details');crosswordHistory.append(element('summary','Recent completions'));const crosswordList=element('ol');for(const r of records.crossword.recent)crosswordList.append(element('li',new Date(r.finished).toLocaleDateString()+' · '+time(r.time)));if(!crosswordList.children.length)crosswordList.append(element('li','Complete the Violet Crossword to start your history.'));crosswordHistory.append(crosswordList);crossword.append(crosswordHistory,element('p','Find the crossword in the Violet Highlands. Completed puzzles record your time, with no XP reward.'));
...
function renderBoard(){if(!$('minigameLeaderboardRows'))return;const [,,kind]=games.find(g=>g[0]===selected);$('minigameLeaderboardType').textContent=kind;
 ...
 else for(const r of rows){const score=['sled','boatSolo','maze9','maze13','maze86','crossword'].includes(selected)?time(r.time):selected==='boat'?time(r.bestTime)+' · '+r.wins+' wins':r.wins+' wins · '+r.games+' games';list.append(element('li',r.name+(r.self?' (you)':'')+' · '+score));}
 $('minigameLeaderboardNote').textContent=['sled','boatSolo','maze9','maze13','maze86','crossword'].includes(selected)?'Player-reported times from account saves, not server-verified. Maze layouts and crossword puzzles may vary. Rankings grant no XP or rewards.':'Completed multiplayer results recorded by the game server. Leaving early does not count as a finish. Rankings grant no extra XP or rewards.'+(selected==='chess'?' Historical chess profiles are retained.':'');
}

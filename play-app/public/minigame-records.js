...
const kinds=['sled','boat','maze9','maze13'];
...
export function createMinigameRecords(storage){
 ...
 const entry={time:run.time,finished:Date.now()};if(kind==='sled'){entry.gates=run.gates;entry.penalty=run.penalty;entry.hits=run.hits;r.bestGates=r.bestGates===null?run.gates:Math.max(r.bestGates,run.gates);}else if(kind==='boat'){entry.penalty=run.penalty;}else{entry.steps=run.steps;entry.seed=run.seed>>>0;r.totalSteps+=run.steps;r.bestSteps=r.bestSteps===null?run.steps:Math.min(r.bestSteps,run.steps);}

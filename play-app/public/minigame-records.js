...
export const BOAT_BEST_KEY='pawtheon-tide-run-best-river-v3';
const kinds=['sled','boat','maze9','maze13','maze86','crossword'];
...
export function createMinigameRecords(storage){
 ...
 let boatLegacy=null;try{const n=Number(storage.getItem(BOAT_BEST_KEY));if(Number.isFinite(n)&&n>0)boatLegacy=n;}catch{saved=false;}
 if(boatLegacy!==null)result.boat.bestTime=result.boat.bestTime===null?boatLegacy:Math.min(boatLegacy,result.boat.bestTime);
 return{records:structuredClone(result),saved};
 }
 function finish(kind,run){if(!kinds.includes(kind)||!number(run.time))throw Error('Invalid minigame result');if(kind.startsWith('maze')&&(!Number.isInteger(run.steps)||run.steps<1))throw Error('Invalid maze steps');if(kind==='sled'&&(!Number.isInteger(run.gates)||run.gates<0||run.gates>9||!number(run.penalty)||!number(run.hits)))throw Error('Invalid sled result');
 if(kind==='crossword'&&(typeof run.puzzle!=='string'||!/[a-zA-Z0-9]/.test(run.puzzle)||run.puzzle.length>80))throw Error('Invalid crossword result');
 if(kind==='boat'&&!number(run.penalty))throw Error('Invalid boat result');
 ...
 const entry={time:run.time,finished:Date.now()};if(kind==='sled'){entry.gates=run.gates;entry.penalty=run.penalty;entry.hits=run.hits;r.bestGates=r.bestGates===null?run.gates:Math.max(r.bestGates,run.gates);}else if(kind==='boat'){entry.penalty=run.penalty;}else if(kind==='crossword'){entry.puzzle=run.puzzle;}else{entry.steps=run.steps;entry.seed=run.seed>>>0;r.totalSteps+=run.steps;r.bestSteps=r.bestSteps===null?run.steps:Math.min(r.bestSteps,run.steps);}

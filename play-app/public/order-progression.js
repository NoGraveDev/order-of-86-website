...
const SHOP='pawtheon-moon-shop-v1',MAX_XP=levelFloor(50),REWARD=50;
...
export function readOrders(raw){if(raw===undefined)raw=emptyOrders();const r=structuredClone(raw);if(!r||r.version!==1||Object.keys(r).some(k=>!['version','creditedXP','collectionXP','activityXP','contracts','upgrades','loadouts'].includes(k)))throw Error('Invalid Order save.');
 ...
 if(!r.contracts||Object.keys(r.contracts).length!==6||!ORDER_IDS.every(o=>{const c=r.contracts[o];return c&&Object.keys(c).every(k=>['completed','step','lastAt','xp'].includes(k))&&integer(c.completed,10000)&&integer(c.step,c.xp===undefined?2:7)&&integer(c.lastAt,1e15)&&(c.xp===undefined||integer(c.xp,c.completed*800));}))throw Error('Invalid Order contract.');
 // Legacy completions keep their earned XP; unfinished local surveys restart on the world route.
 for(const c of Object.values(r.contracts))if(c.xp===undefined){c.xp=c.completed*800;c.step=0;c.lastAt=0;}
...
export function contractRoute(order,completed=0){
 const pool=realms.slice(1).sort((a,b)=>a.angle-b.angle),start=(pool.findIndex(r=>r.id===ORDER_REALMS[order])+completed)%pool.length;
 // Stride three crosses the world between stops, visiting all seven realms once, then home.
 return [...Array.from({length:pool.length},(_,i)=>pool[(start+i*3)%pool.length]),realms[0]];
}

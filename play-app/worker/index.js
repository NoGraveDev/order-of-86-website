...
export default {async fetch(request,env){const url=new URL(request.url);if(url.pathname.startsWith('/api/accounts/'))return accountRequest(request,env.DB);if(!url.pathname.startsWith('/api/multiplayer/'))return env.ASSETS.fetch(request);if(request.method!=='POST')return json({error:'Use POST.'},405);if(request.headers.get('Origin')&&request.headers.get('Origin')!==url.origin)return json({error:'Origin rejected.'},403);if(!env.DB)return json({error:'Multiplayer is temporarily unavailable. Solo play is unaffected.'},503);
 try{const raw=await request.text();if(raw.length>4096)return json({error:'Request too large.'},413);const body=JSON.parse(raw),action=url.pathname.split('/').at(-1),now=Date.now();const db=env.DB;const playerAccount=await currentAccount(request,db,now);if(!playerAccount)return json({error:'Sign in to play Pawtheon.'},401);
 ...
 const account=playerAccount,name=account.name;
 ...
 const profile='account:'+account.id;
 ...
 const membership=await db.prepare('SELECT profile FROM multiplayer_chess_members WHERE token=?').bind(token).first();if(membership?.profile!=='account:'+playerAccount.id)return json({error:'Sign in to the account that joined this room.'},401);

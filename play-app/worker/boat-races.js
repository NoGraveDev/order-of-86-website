import {captureBoatResults} from './minigame-records.js';
...
export async function boatRequest(db,room,token,body,now){
 ...
 await captureBoatResults(db,room,race,roster);
 ...
 if(action==='join'||action==='new'){
  ...
  await db.prepare("UPDATE multiplayer_boat_entries SET status='dnf',updated=? WHERE room=? AND player=? AND race=? AND status IN ('ready','racing')").bind(now,room,self,race.id).run();
